...
# draw a line chart
def DrawFigure(x_values, y_values, legend_labels, x_label, y_label, filename, allow_legend, color_map = COLOR_MAP, patterns = PATTERNS):
  ...
  for i in range(len(y_values)):
    bars[i] = plt.bar(index + width / 2, y_values[i], width, hatch=patterns[i], color=color_map[i], label=legend_labels[i], bottom=bottom_base)
