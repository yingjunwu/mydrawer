...
# draw a line chart
def DrawFigure(x_values, y_values, legend_labels, x_label, y_label, folder, filename, allow_legend, color_map = COLOR_MAP, patterns = PATTERNS):
  ...
  for i in range(len(y_values)):
    bars[i] = figure.bar(index, y_values[i], width, hatch=patterns[i], color=color_map[i], label=legend_labels[i], edgecolor="black", bottom=bottom_base)
  ...
  plt.savefig(folder + "/" + filename + ".pdf", bbox_inches='tight', format='pdf')
